...
class Model(base.Model):
    ...
    def encode_vl(self, vocab, **inputs):
        ...
        if frames is not None:
            ...
            emb_frames = torch.zeros([batch_size, 0,
                                      self.args.demb]).to(device)
            lengths_frames, length_frames_max = torch.tensor([0] *
                                                             batch_size), 0
    ...
    def encode_lang(self, vocab, lang_pad):
        ...
        emb_lang, lengths_lang = self.encoder_lang(lang_pad, embedder_lang,
                                                   vocab, self.pad)
        emb_padding = torch.zeros(emb_lang.shape[:2],
                                  device=emb_lang.device).bool()
    ...
    def forward(self, vocab, **inputs):
        ...
        target_mask = model_util.triangular_mask(lang_target.size(1),
                                                 lang_target.device)
        ...
        output = {
            "lang":
            lang_out_flat.view(len(decoder_out), -1, lang_out_flat.shape[-1])
        }
        return output
    ...
    def compute_metrics(self, model_out, gt_dict, metrics_dict, verbose=False):
        ...
        pred_lang = model_util.tokens_to_lang(pred_tokens,
                                              self.vocab_out, {self.pad},
                                              join=False)
        gt_lang = model_util.tokens_to_lang(gt_dict["action"],
                                            self.vocab_out, {self.pad},
                                            join=False)
        pred_lang_strs = [" ".join(s) for s in pred_lang]
        gt_lang_strs = [" ".join(s) for s in gt_lang]
        model_util.compute_f1_and_exact(metrics_dict, pred_lang_strs,
                                        gt_lang_strs, "lang")
        if verbose:
            logger.debug("Lang GT:\n{}".format(gt_lang_strs[0]))
            logger.debug("Lang predictions:\n{}".format(pred_lang_strs[0]))
            logger.debug("EM = {}, F1 = {}".format(
                metrics_dict["lang/exact"][-1], metrics_dict["lang/f1"][-1]))
    ...
    def translate(self, vocab_in, max_decode=300, num_pad_stop=3, **inputs):
        ...
        """
        # prepare
        batch_size = len(inputs["lang"] if "lang" in
                         inputs else inputs["frames"])
        device = (inputs["lang"]
                  if "lang" in inputs else inputs["frames"]).device
        # pass inputs to the encoder
        ...
        for i in range(max_decode):
            ...
            decoder_out_flat = decoder_out.reshape(-1, self.args.demb)
            lang_out_flat = decoder_out_flat.mm(self.emb_subgoal.weight.t())
            lang_out = lang_out_flat.view(batch_size, -1,
                                          lang_out_flat.shape[-1])
        ...
        lang_result = [[t for t in tokens if t != self.pad]
                       for tokens in lang_result]
        return lang_result
