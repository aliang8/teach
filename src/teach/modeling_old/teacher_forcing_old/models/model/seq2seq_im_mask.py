...
class Seq2SeqFollowerAgent(Base):
    ...
    def featurize(self, batch, load_mask=True, load_frames=True):
        ...
        for ex in batch:
            ...
            lang_goal, lang_instr = ex['num']['lang_goal'][0], ex['num'][
                'lang_instr']
            ...
            for t in range(len(ex["ann"]["utter_t"]) + 1):
                ...
                if t == 0:
                    ...
                    repeat = ex["ann"]["utter_t"][t] - ex["ann"]["utter_t"][t -
                                                                            1]
                # print(t, ex["ann"]["utter_t"], repeat, max_t)
                seq = list(
                    torch.tensor(lang_goal_instr,
                                 device=device).repeat(repeat, 1))
                utter += seq
            ...
            if load_frames and not self.test_mode:
                ...
                num_low_actions = len(ex['plan']['low_actions']
                                      ) + 1  # +1 for additional stop action
            ...
            if not self.test_mode:
                ...
                feat['action_low'].append(
                    [a['action'][0] for a in ex['num']['driver_actions_low']])
                ...
                feat['action_low_coord'].append(
                    [[a['x'], a['y']] for a in ex['num']['interactions']
                     if 'x' in a if a['success']])

                # low-level valid interact
                # TODO: i think this is only interact actions
                feat['action_low_valid_interact'].append([
                    a['success'] for a in ex['num']['driver_actions_low']
                    if 'x' in a
                ])
    ...
    def serialize_lang_action(self, feat):
        ...
        if not is_serialized:
            ...
            if not self.test_mode:
                feat['num']['action_low'] = [
                    a['action_id'] for a in feat['num']['interactions']
                ]
    ...
    def extract_preds(self, out, batch, feat, clean_special_tokens=True):
        ...
        '''
        pred = {}
        for ex, alow, alow_mask, alow_coord in zip(
                batch, feat['out_action_low'].max(2)[1].tolist(),
                feat['out_action_low_mask'], feat['out_action_low_coord']):
            ...
            p_mask = [(alow_mask[t] > 0.5).cpu().numpy()
                      for t in range(alow_mask.shape[0])]
    ...
    def embed_action(self, action):
        ...
        action_num = torch.tensor(self.vocab['action_low'].word2index(action),
                                  device=device)
    ...
    def compute_loss(self, out, batch, feat):
        ...
        alow_coord_loss = self.mse_loss(flat_p_alow_coord,
                                        flat_alow_coord).mean()
