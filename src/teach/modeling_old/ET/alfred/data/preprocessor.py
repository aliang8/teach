...
class Preprocessor(object):
    ...
    def process_language(self, ex, traj, r_idx, is_test_split=False):
        ...
        goal_desc = revtok.tokenize(
            data_util.remove_spaces_and_lower(goal_desc))
        goal_desc = [w.strip().lower() for w in goal_desc]

        # instr_anns = [utterance for (speaker, utterance) in ex["dialog_history"]]
        instr_anns = [
            revtok.tokenize(data_util.remove_spaces_and_lower(instr_ann))
    ...
    def tokenize_and_numericalize(self,
                                  dialog_history,
                                  numericalize=True,
                                  train=False):
        ...
        if numericalize:
            instr = [
                self.numericalize(self.vocab["word"], word, train=train)
                for word in instr
            ]
    ...
    def process_actions(self, ex, traj):
        ...
        for action in traj['tasks'][0]['episodes'][0]['interactions']:
            ...
            action_idx = action_to_action_idx[str(
                action_dict_with_idx['action_id'])]
            action_name = idx_to_name[str(action_idx)]

            action_dict_with_idx["action"] = (
                self.vocab["action_low"].word2index(action_name, train=True), )
            action_dict_with_idx["action_name"] = action_name

            if action_dict_with_idx['agent_id'] == 0:
                traj["num"]["commander_actions_low"].append(
                    action_dict_with_idx)
            else:
